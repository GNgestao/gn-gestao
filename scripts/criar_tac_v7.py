...
# Extrai cada cookie separadamente sem concatenar
js_extrair_cookies = """
const resp = $json;
...
# Buscar chamados usa campos separados para montar o cookie header
js_montar_mensagem = """
const body = $json;
let lista = [];
try {
        ...
        "nodes": [
            ...
            },
            {
                "parameters": {
                    ...
                    "sendHeaders": True,
                    "headerParameters": {"parameters": [
                        {"name": "Cookie", "value": "=ASP.NET_SessionId={{ $json.s }}; LOGIN={{ $json.l }}; TKEMobile={{ $json.t }}; USER={{ $json.u }}"},
                        {"name": "Content-Type", "value": "application/json; charset=UTF-8"},
                        {"name": "X-Requested-With", "value": "XMLHttpRequest"},
                        {"name": "User-Agent", "value": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/148.0.0.0 Safari/537.36"},
                        {"name": "Origin", "value": "https://mobile.br.tkelevator.com"},
...
deletar("4FnXS2oEdblKVCmT")
print("Fluxo antigo removido")
wid, err = criar(wf_tac_v7())
if wid:
    ok = ativar(wid)
